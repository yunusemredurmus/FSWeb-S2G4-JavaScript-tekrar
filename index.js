...
/**
 *  Çemberin çevre hesabı ! 
 * @param {CemberinCevresi(8)} yaricap1
 * @returns Çemberin çevresini döndürüyor
 */
...
/**
 * Çemberin alanı hesaplama
 * @param {yaricap<number>} yaricap 
 * @param {pi} sayiPi 
 * @returns Çemberin alanını döndürür.
 */

function CemberinAlani(yaricap, sayiPi) {
	const capKare = Math.pow(yaricap, 2);
	return sayiPi * capKare
}
...
/** Array içerisindeki en büyük ve en küçük değerleri aldık burada. enbuyuk,enkucuk değerlere neden sayilar [0] verdik çünkü if koşulunda bir karşılaştırma 
 *  yapacağız ve bir değer atamalıydık ki listede ki en yüksek sayı ile kıyaslama yapabilsin
 */
...
/** 3 rakamına tam bölünebilen sayıların içerisinde bulunduğu bir dize oluşturduk. 
 * Sayilar arrayini kullandık ve forEach metoduyla birlikte array içerisinde dolaşıma çıktık daha sonrasında if koşuluyla kalan 0 olan bütün sayıları dahil ettik.
 */
...
console.log("3'e tam bölünebilen sayılar : ", ucetambolunenler);


//3c çözümü:

/** Bi önceki bölünebilen sayıları 
 * .reduce methoduyla birbirleriyle topladık ve sonucu aşağıya yazdırdık.
 */

ucebolunenlerintoplami = ucetambolunenler.reduce((sayi, sira) => sayi + sira, 0);
console.log("3'e bölünebilen sayıların toplamı : ", ucebolunenlerintoplami);

//3d çözümü
/** sayilar dizesindeki 500 sayısından küçük sayıları ile birlikte besyundenkucuksayilar
 * besyundenkucuksayilar dizisini oluşturduk.
 */
...
//3e çözümü
/** besyuzdenkucuksayilar dizesni siralisayilar dizesinde küçükten büyüğe
 * sıraladık. :) 
 */
siralisayilar = [];
...
//3f çözümü
/**
 * Dize içerisindeki tekrar eden sayıları ve kaç kere tekrar edildiğini bul!
 */
tekraredensayilar = [];

for (let i = 0; i < sayilar.length; i++) {
	for (let j = i + 1; j < sayilar.length; j++) {
		if (sayilar[i] === sayilar[j] && !tekraredensayilar.includes(sayilar[i])) {
			tekraredensayilar.push(sayilar[i]);
		}
	}
	for (let i = 0; i < tekraredensayilar.length; i++) {
		let tekrarSayisi = sayilar.filter(sayi => sayi === tekraredensayilar[i]).length;
		console.log(tekraredensayilar[i] + " sayısı " + tekrarSayisi + " kez tekrar edilmiştir.");
